...
RESULTS_DIR = BASE_DIR / "results_boundary_sampled_3class"
...
core_clusters = set(real_df["cluster"].dropna().unique())

# Core-anchored synthetic
synth_core = synth_df[synth_df["cluster"].isin(core_clusters)]

# Boundary synthetic = non-noise synthetic outside core clusters
synth_boundary = synth_df[~synth_df["cluster"].isin(core_clusters)]

print("\nReal data distribution:")
print(real_df["function"].value_counts())

print("\nBoundary synthetic distribution:")
print(synth_boundary["function"].value_counts())
...
results_df.to_csv(
    RESULTS_DIR / "results_boundary_sampled_runs.csv",
    index=False
)

summary_df = results_df.describe().loc[["mean", "std"]]
summary_df.to_csv(
    RESULTS_DIR / "results_boundary_sampled_summary.csv"
)
...
conf_df.to_csv(
    RESULTS_DIR / "confusion_boundary_sampled.csv"
)

print("\n Boundary-sampled 3-class experiment completed and saved.")

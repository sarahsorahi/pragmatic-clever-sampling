...
RESULTS_DIR = BASE_DIR / "results_real_only_3class"
...
for fname in [
    ...
    dfs.append(df[df["Label"].isna()])  # real only
...
print("\nReal-only data distribution (3-class):")
print(real_df["function"].value_counts())
print("Total real examples:", len(real_df))
...
for seed in SEEDS:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
        ...
        train_df = real_df.iloc[train_idx]
...
results_df.to_csv(RESULTS_DIR / "results_real_only_runs.csv", index=False)

summary_df = results_df.describe().loc[["mean", "std"]]
summary_df.to_csv(RESULTS_DIR / "results_real_only_summary.csv")

conf_df = pd.DataFrame(conf_matrix_total, index=LABELS, columns=LABELS)
conf_df.to_csv(RESULTS_DIR / "confusion_real_only.csv")

print("\n Real-only 3-class experiment completed and saved.")

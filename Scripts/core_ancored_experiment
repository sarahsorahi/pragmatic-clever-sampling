...
RESULTS_DIR = BASE_DIR / "results_core_anchored_3class"
...
for fname in [
    ...
    # synthetic candidates (non-noise)
    dfs_synth.append(df[(df["Label"] == "L") & (df["cluster"] != -1)])
...
print("\nSynthetic (non-noise) candidates:")
print(synth_df["function"].value_counts())

# =========================
# Core-anchored filtering
# =========================

# Clusters that contain at least one real example
core_clusters = set(
    real_df["cluster"].dropna().unique()
)

synth_core = synth_df[synth_df["cluster"].isin(core_clusters)]

print("\nCore-anchored synthetic distribution:")
print(synth_core["function"].value_counts())
...
for seed in SEEDS:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
        ...
        train_df = pd.concat([train_real, synth_core], ignore_index=True)
...
results_df.to_csv(RESULTS_DIR / "results_core_anchored_runs.csv", index=False)

summary_df = results_df.describe().loc[["mean", "std"]]
summary_df.to_csv(RESULTS_DIR / "results_core_anchored_summary.csv")

conf_df = pd.DataFrame(conf_matrix_total, index=LABELS, columns=LABELS)
conf_df.to_csv(RESULTS_DIR / "confusion_core_anchored.csv")

print("\n Core-anchored 3-class experiment completed and saved.")
